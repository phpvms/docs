...
To add a new remote disk like Amazon S3, CloudFlare R2 or SFTP, you must configure the details in `.env` properly. As an example, for CloudFlare R2 a bucket should be configured in your account along with an S3 API and below details should be in your `.env` file
...
Also it is possible to add more disks to support different file systems, you need to create the disk in `config/filesystems.php`. Examples of configuring disks can be found [here](https://laravel.com/docs/10.x/filesystem#driver-prerequisites) and other online sources.
