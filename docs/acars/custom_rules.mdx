...
For users with Premium, you can create their own rules, in Javascript. See [Custom Packaging](custom_packaging) on how to create a distribution for your VA, which can include rules, sounds and callbacks.
...
- There is no `setTimeout` or `async`/`await` calls
-

## Built-In Methods available

You can also browse the built-in methods available in the `mocks.js` file
...
```js
return violatedAfterDelay("landing_lights_on", 5000, () => {
