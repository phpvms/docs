...
central index of community addons. Once your addon is registered, operators can
discover and install it directly from their phpvms admin UI — they never need to
clone repos or run composer commands.
...
- The zip's `module.json` declares a **`registry_id`** equal to your registry
  name (for example, `acme/reports`). See
...
- The `{author}` segment should match your **GitHub username or organisation**.
  This keeps ownership unambiguous and makes the source repo easy to find.
- The `{package}` segment should match the **GitHub repository name** of the
  addon source. So an addon hosted at `https://github.com/acme/reports-addon` is
  best registered as `acme/reports-addon`.
...
5. **Zip integrity** — downloadable, contains `module.json` at the root, no
   forbidden paths (`.git/`, `.github/`, `tests/`, `node_modules/`, `.idea/`,
...
1. The `release-block` workflow opens an auto-merging bot PR appending the
   resolved `release:` block to your YAML.
